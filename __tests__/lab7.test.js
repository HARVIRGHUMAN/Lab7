describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    */
    for (let i = 0;i < prodItemsData.length;i++) {
      ...
      if (!value.price || String(value.price).length === 0) { allArePopulated = false; }
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    // Start at i = 1 because the first item's button was already clicked in Step 2
    for (let i = 1; i < prodItems.length; i++) {
      const shadowRoot = await prodItems[i].getProperty('shadowRoot');
      const button = await shadowRoot.$('button');
      
      // Use page.evaluate to trigger the native DOM click event directly inside the browser
      await page.evaluate(btn => btn.click(), button);
    }

    const cartCountElement = await page.$('#cart-count');
    const cartCountText = await page.evaluate(el => el.innerText, cartCountElement);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // Check all product-item buttons directly inside the browser execution context
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    // Click every single button via page.evaluate to maximize speed and bypass timeouts
